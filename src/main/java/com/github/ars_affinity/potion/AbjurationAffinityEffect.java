...
public class AbjurationAffinityEffect extends MobEffect {
    
    public AbjurationAffinityEffect() {
    ...
    public boolean applyEffectTick(LivingEntity entity, int amplifier) {
        if (entity instanceof Player player) {
            ...
            if (affinityProgress != null) {
                float currentAffinity = affinityProgress.getAffinity(SpellSchools.ABJURATION);
                float newAffinity = Math.min(1.0f, currentAffinity + 0.10f); // Increase by 10%
                affinityProgress.setAffinity(SpellSchools.ABJURATION, newAffinity);
                
                ArsAffinity.LOGGER.info("ABJURATION AFFINITY - Applied abjuration affinity potion effect for player: {} - affinity: {} -> {}", 
    ...
    public String getDescriptionId() {
        return "effect.ars_affinity.abjuration_affinity";
    }
}