...
public class WaterAffinityEffect extends MobEffect {
    ...
    public boolean applyEffectTick(LivingEntity entity, int amplifier) {
        if (entity instanceof Player player) {
            ...
            if (affinityProgress != null) {
                float currentAffinity = affinityProgress.getAffinity(SpellSchools.ELEMENTAL_WATER);
                float newAffinity = Math.min(1.0f, currentAffinity + 0.10f); // Increase by 10%
                affinityProgress.setAffinity(SpellSchools.ELEMENTAL_WATER, newAffinity);
                
                ArsAffinity.LOGGER.info("WATER AFFINITY - Applied water affinity potion effect for player: {} - affinity: {} -> {}", 
    ...
    public String getDescriptionId() {
        return "effect.ars_affinity.water_affinity";
    }
}