...
public class ManipulationAffinityEffect extends MobEffect {
    ...
    public boolean applyEffectTick(LivingEntity entity, int amplifier) {
        if (entity instanceof Player player) {
            ...
            if (affinityProgress != null) {
                float currentAffinity = affinityProgress.getAffinity(SpellSchools.MANIPULATION);
                float newAffinity = Math.min(1.0f, currentAffinity + 0.10f); // Increase by 10%
                affinityProgress.setAffinity(SpellSchools.MANIPULATION, newAffinity);
                
                ArsAffinity.LOGGER.info("MANIPULATION AFFINITY - Applied manipulation affinity potion effect for player: {} - affinity: {} -> {}", 
    ...
    public String getDescriptionId() {
        return "effect.ars_affinity.manipulation_affinity";
    }
}